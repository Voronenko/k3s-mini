...
resource "esxi_guest" "cluster_master" {
  ...
  numvcpus           = "2" # Optional - Number of virtual cpus. See esxi documentation for limits. - Default 1 or default taken from cloned source.
...
resource "esxi_guest" "cluster_node01" {
  count              = 0
  guest_name         = "cluster_node01"
  notes              = "k3s cluster"  # Optional - The Guest notes (annotation).
  disk_store         = var.disk_store # Required - esxi Disk Store where guest vm will be created
  boot_disk_type     = "thin" # Optional - Guest boot disk type. Default 'thin'. Available thin, zeroedthick, eagerzeroedthick
...
resource "esxi_guest" "cluster_node02" {
  count              = 0
  guest_name         = "cluster_node02"
  notes              = "k3s"  # Optional - The Guest notes (annotation).
  disk_store         = var.disk_store # Required - esxi Disk Store where guest vm will be created
  boot_disk_type     = "thin" # Optional - Guest boot disk type. Default 'thin'. Available thin, zeroedthick, eagerzeroedthick
  boot_disk_size     = "30" # Optional - Specify boot disk size or grow cloned vm to this size.
